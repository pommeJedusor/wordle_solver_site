...
export default function Home() {
  const cheatButtonRef = useRef<HTMLButtonElement | null>(null)
  const resetButtonRef = useRef<HTMLButtonElement | null>(null)

  function unfocusResetGameButton(){
    resetButtonRef.current?.blur();
  }
  function unfocusCheatButton(){
    cheatButtonRef.current?.blur();
  }
  ...
  return (
      <div className="flex flex-col items-center gap-10 dark:bg-background-night bg-background-day min-h-dvh">
        <div className="flex flex-row justify-center mt-10">
          <button ref={cheatButtonRef} onClick={()=>{toggleDailyWordleMode();unfocusCheatButton()}} type="button" title="change color according to today's word on wordle" className="cursor-pointer mx-2 focus:outline-none hover:outline-none focus:ring-4 hover:ring-4 focus:ring-yellow-700 dark:focus:ring-yellow-800 hover:ring-yellow-600 dark:hover:ring-yellow-700 bg-yellow-500 dark:bg-yellow-600 hover:bg-yellow-400 dark:hover:bg-yellow-500 p-1 rounded" >
            <Image className="md:w-12 sm:w-9 w-6 md:h-12 sm:h-9 h-6 mx-auto" width={500} height={500} src="/star.svg" alt="toggle cheat" />
          </button>
          <button ref={resetButtonRef} onClick={()=>{resetGame();unfocusResetGameButton()}} type="button" title="refresh" className="cursor-pointer mx-2 focus:outline-none hover:outline-none focus:ring-4 hover:ring-4 focus:ring-sky-700 dark:focus:ring-sky-800 hover:ring-sky-600 dark:hover:ring-sky-700 bg-sky-500 dark:bg-sky-600 hover:bg-sky-400 dark:hover:bg-sky-500 p-1 rounded" >
